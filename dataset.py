...
class ClipCocoDataset(Dataset):
    ...
    def pad_tokens(self, item: int):
        tokens = self.captions_tokens[item][:-1]
        labels = self.captions_tokens[item][1:]
    ...
    def __init__(self, data_path: str,  tokenizer, padding=True, normalize_features=False):
        ...
        for caption in captions_raw:
            self.captions_tokens.append(torch.tensor(self.tokenizer.encode('<bos>' + caption['caption'] + ' <eos>'), dtype=torch.int64))
