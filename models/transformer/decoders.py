...
class MeshedDecoder(Module):
    def __init__(self, vocab_size, max_len=30, N_dec=6, N_enc=6, padding_idx=0, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048,
                 dropout=.1,
...
class Decoder(Module):
    def __init__(self, vocab_size=50257, max_len=40, N_dec=6, padding_idx=0, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048,
                 dropout=.1, return_states=False, self_att_module=None, enc_att_module=None, self_att_module_kwargs=None,
    ...
    def forward(self, input, encoder_output, mask_encoder):
        ...
        for i, l in enumerate(self.layers):
            ...
            hidden_state_list.append(out)
        logits = self.fc(out) 

        if self.return_states == False:
            return logits 
        else:
            return (logits, hidden_state_list)
