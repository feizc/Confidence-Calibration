...
from models import ConfidenceEstimationFramework, TransformerConfig
...
def confidence_estimation_loss(caption_prob, confidence_prob, labels, one_hot_labels, alpha=1.0): 
    combine_prob = caption_prob * confidence_prob + (1 - confidence_prob) * one_hot_labels
    xe_loss = nnf.nll_loss(combine_prob.reshape(-1, combine_prob.shape[-1]), labels.flatten(), ignore_index=0) 
    conf_loss = torch.log(confidence_prob).sum() 
    return xe_loss + alpha * conf_loss 
...
def train_xe(model, train_dataloader, args, optimizer, scheduler, epoch): 
    model.train()
    running_loss = .0 
    progress = tqdm(total=len(train_dataloader), desc='ConfidenceEstimation') 
    for idx, (tokens, labels, _, img_features) in enumerate(train_dataloader):  
        ...
        caption_prob, confidence_prob  = model(img_features, tokens)  # (bsz, seq, vob_size) 
        one_hot_labels = nnf.one_hot(labels, model.captioner.vocab_size) 
        loss = confidence_estimation_loss(caption_prob, confidence_prob, labels, one_hot_labels)
        loss.backward()
        optimizer.step()
        scheduler.step()
        optimizer.zero_grad()
...
def main(): 
    ...
    model = ConfidenceEstimationFramework(config).to(device) 
